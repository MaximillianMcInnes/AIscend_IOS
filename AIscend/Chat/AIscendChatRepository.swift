...
private extension AIscendChatRepository {
    ...
    static func decodeMessages(from data: [String: Any]) -> [AIscendChatMessage] {
        if let jsonString = data["json"] as? String, let decoded = decodeMessageString(jsonString), !decoded.isEmpty {
            return decoded
        }

        if let rawMessages = data["messages"] as? [[String: Any]], !rawMessages.isEmpty {
            return rawMessages.compactMap(parseMessage(from:))
        }

        if let jsonMessages = data["json"] as? [[String: Any]], !jsonMessages.isEmpty {
            return jsonMessages.compactMap(parseMessage(from:))
        }

        if let messageString = data["messages"] as? String, let decoded = decodeMessageString(messageString), !decoded.isEmpty {
            return decoded
        }
