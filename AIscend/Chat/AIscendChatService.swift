...
protocol AIscendChatServiceProtocol: Sendable {
    func sendQuery(message: String, email: String, filters: AIscendChatFilters?) async throws -> AIscendChatRAGResult
...
actor AIscendChatService: AIscendChatServiceProtocol {
    ...
    func sendQuery(message: String, email: String, filters: AIscendChatFilters?) async throws -> AIscendChatRAGResult {
        ...
        let resolvedEmail = identity.email?.lowercased() ?? email.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
