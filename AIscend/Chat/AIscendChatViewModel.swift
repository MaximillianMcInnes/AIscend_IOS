...
final class AIscendChatViewModel {
    ...
    var currentTitle: String {
        if messages.isEmpty && activeThreadID == nil {
    ...
    var isAuthenticated: Bool {
        session.phase == .signedIn && userEmail != nil
    }
    ...
    var showEmptyState: Bool {
        isAuthenticated && messages.isEmpty && !isBootstrapping
    }
    ...
    func loadWorkspace() async {
        ...
        do {
            async let loadedThreads = repository.loadThreads(for: userEmail, userID: userID)
            async let repositoryQuota = repository.loadQuota(for: userEmail, userID: userID)
    ...
    func sendCurrentDraft() async {
        ...
        do {
            ...
            await refreshQuota(for: userEmail)

            do {
                let refreshedThreads = try await repository.loadThreads(for: userEmail, userID: userID)
...
private extension AIscendChatViewModel {
    ...
    func refreshQuota(for email: String) async {
        async let repositoryQuota = repository.loadQuota(for: email, userID: userID)
        async let authQuota = service.loadAuthQuotaSnapshot()
