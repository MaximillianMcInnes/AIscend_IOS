...
struct AIscendChatIdentity: Equatable, Sendable {
    let userID: String
    let email: String?
    ...
    init?(userID: String?, email: String?) {
        ...
        self.email = trimmedEmail.isEmpty ? nil : trimmedEmail.lowercased()
    }

    var requestEmail: String {
        email ?? "\(userID)@private.aiscend.local"
    }
}
...
struct AIscendChatThread: Identifiable, Equatable {
    ...
    var hasRenderableMessages: Bool {
...
enum AIscendChatWorkspaceState: Equatable {
    case signedOut
    case loading
    case empty
    case draft
    ...
    static func resolve(
        ...
        threads: [AIscendChatThread],
        messages: [AIscendChatMessage],
        prefersFreshConversation: Bool
    ) -> AIscendChatWorkspaceState {
        if !isAuthenticated {
            return .signedOut
        }

        if messages.contains(where: { !$0.text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }) {
            return .conversation
        }
        ...
        if threads.contains(where: \.hasRenderableMessages) || !threads.isEmpty {
            return .recovery
        }
