...
final class ScanResultsViewModel: ObservableObject {
    ...
    var scoreCards: [ResultsMetricCardModel] {
        ...
        return [
            ...
            ResultsMetricCardModel(
                ...
                value: scoreText(
                    isPremium
                    ? (result.payload.scores.jaw ?? result.overallScore - 1)
                    : (result.payload.scores.skin ?? result.overallScore - 4)
                ),
    ...
    func load(initialResult: PersistedScanRecord? = nil) async {
        ...
        let resolvedResult: PersistedScanRecord?
        if let initialResult {
            resolvedResult = initialResult
        } else {
            resolvedResult = await repository.loadLatestPersistedResult()
        }
    ...
    func subtitle(for page: ScanResultsPageID) -> String {
        ...
        case .placement:
            return result.placementNarrative
        case .harmony:
            return "AIScend is reading how the face holds together as a whole, not just how isolated features score."
        case .eyes:
            return isPremium
        ...
        case .lips:
            return "This section keeps the mouth area concise and useful so the result feels clear instead of noisy."
        case .jaw:
            return "Jawline structure changes how the full face reads. Premium goes deeper into leverage and visual payoff."
        case .sideProfile:
            return "The side profile often changes the total impression more than expected. This page isolates that angle cleanly."
        case .premiumPush:
            return "Move from preview mode into the complete report with jaw, skin, side profile detail, and deeper guidance."
        case .done:
            switch autoSaveState {
            case .saved:
                return "The result has been archived and is ready for the next move."
            default:
                return "The result is ready to use, revisit, and turn into an actual plan."
            }
        }
    }
    ...
    func harmonyTraits() -> [ScanTraitRowModel] {
        ...
        return [
            trait(
                ...
                explanation: "The face reads composed when the major planes stay orderly relative to each other."
            ),
            trait(
                label: "Feature balance",
                value: qualitativeLabel(for: result.overallScore),
                explanation: "The strongest global improvement now comes from keeping each feature working in the same direction."
            ),
            trait(
                label: "Lower-third composition",
                value: qualitativeLabel(for: result.payload.scores.jaw ?? result.overallScore - 2),
                explanation: "The lower third has enough structure to keep the read masculine without feeling forced."
            ),
            trait(
                label: "Presentation coherence",
                value: qualitativeLabel(for: result.potentialScore),
    ...
    func makeTraitRow(key: String, value: ScanJSONValue) -> ScanTraitRowModel? {
        let normalizedLabel = PersistedScanRecord.normalizedLabel(
            for: key.components(separatedBy: ".").last ?? key
        )
